...
namespace TaxiService
{
    ...
    {
        #region CreateAllBases
        public const int AmountTaxists = 300;
        public const int AmountUsers = 300;
        public const int AmountCars = AmountTaxists / 3;
        ...
        public static string RandomNumberCar(this Random rand)
        {
            ...
            string number = registAreaCode[rand.Next(Codes)] + registrationNumberStr + seriesNumbers[rand.Next(SeriesNumber)];
            return number;
        }
        ...
        public static void CreateBaseCars()
        {
            ...
            {
                economCars.Add(new EconomCar(rand.RandomNameEconomCar(), rand.RandomModelCar(), rand.RandomNumberCar(), rand.Next(1,5),taxists[i], rand.NextBool(), rand.NextBool()));
            }

            List<LuxuryCar> luxuryCars = new List<LuxuryCar>(AmountCars);
            for (int i = 0; i < luxuryCars.Capacity; ++i)
            {
                luxuryCars.Add(new LuxuryCar(rand.RandomNameLuxuryCar(), rand.RandomModelCar(), rand.RandomNumberCar(), rand.Next(1, 3), taxists[i+economCars.Capacity], rand.NextBool(), rand.NextBool()));
            }

            List<Truck> trucks = new List<Truck>(AmountCars);
            for (int i = 0; i < trucks.Capacity; ++i)
            {
                trucks.Add(new Truck(rand.RandomNameTruck(), rand.RandomModelCar(), rand.RandomNumberCar(), rand.Next(1, 3), taxists[i + economCars.Capacity+luxuryCars.Capacity], rand.Next(10001)));
            }
        ...
        public static void CreateBaseUsers()
        {
            ...
            {
                users.Add(new User(rand.RandomName(),rand.RandomSurname(),rand.RandomStreet()));
            }
            //Econom
            List<EconomCar> economCars = new List<EconomCar>(AmountCars);
            for (int i = 0; i < economCars.Capacity; ++i)
